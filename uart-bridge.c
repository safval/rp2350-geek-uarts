...
void update_uart_cfg(uint8_t itf)
{
  ...
  {
    char parity_char = 'N';
    switch (parity_usb2uart(ud->usb_lc.parity))
    {
    ...
    Show_Params(itf, ud->uart_lc.bit_rate,
      databits_usb2uart(ud->usb_lc.data_bits),
      parity_char,
      stopbits_usb2uart(ud->usb_lc.stop_bits));
  }
  ...
  while (1)
  {
    ...
    for (itf = 0; itf < CFG_TUD_CDC; itf++)
      if (tud_cdc_n_connected(itf))
        ...
        UART_DATA[itf].usb_lc.bit_rate = 0;
  }
}
...
void init_uart_data(uint8_t itf)
{
  ...
  /* USB CDC LC */
  ud->usb_lc.bit_rate = 115200;
  ud->usb_lc.data_bits = 8;
  ud->usb_lc.parity = 0;
  ud->usb_lc.stop_bits = 1;

  /* UART LC */
  ud->uart_lc.bit_rate = 115200;
  ud->uart_lc.data_bits = 8;
  ud->uart_lc.parity = 0;
  ud->uart_lc.stop_bits = 1;
